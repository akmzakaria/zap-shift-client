    ...
    registerUser(data.email, data.password)
      .then(() => {
        ...
        axios.post(imgAPI_URL, formData).then((res) => {
          const phtotoURL = res.data.data.url // or display_url
          console.log('after image upload', phtotoURL)
          ...
          // update user profile to firevbase
          const userProfile = {
            displayName: data.name,
            photoURL: phtotoURL, // or display_url
          }
    ...
    <div className="h-screen flex flex-col justify-center items-center">
      ...
      <form onSubmit={handleSubmit(handleReg)}>
        <fieldset className="fieldset">
          ...
          <button className="btn btn-primary text-black w-90 mt-4">Register</button>
          <p className="text-[#71717a]">
            Already have an account?{' '}
            <Link state={location.state} className="text-primary underline" to={'/login'}>
              Login
            </Link>
          </p>
        </fieldset>
